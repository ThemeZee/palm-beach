...
function palm_beach_theme_addons_scripts() {
	...
	if ( is_active_widget( 'TZWB_Facebook_Likebox_Widget', false, 'tzwb-facebook-likebox' )
		...
		wp_enqueue_style( 'palm-beach-widget-bundle', get_template_directory_uri() . '/assets/css/themezee-widget-bundle.css', array(), '20160421' );
	...
	if ( is_singular( 'post' ) ) {
		...
		wp_enqueue_style( 'palm-beach-related-posts', get_template_directory_uri() . '/assets/css/themezee-related-posts.css', array(), '20160421' );
 ...
 */
function palm_beach_amp_menu_toggle() {
 ...
 */
function palm_beach_amp_menu_is_toggled() {
	if ( palm_beach_is_amp() ) {
 ...
 */
function palm_beach_amp_menu_dropdown_toggles( $item_output, $item, $depth, $args ) {
	...
	*/
	...
	// Add SVG icon.
	$dropdown_button .= $expanded ? palm_beach_get_svg( 'collapse' ) : palm_beach_get_svg( 'expand' );
	...
	$dropdown_button .= sprintf(
		...
		esc_attr( sprintf( "$expanded_state_id ? %s : %s", wp_json_encode( esc_html__( 'Collapse child menu', 'palm-beach' ) ), wp_json_encode( esc_html__( 'Expand child menu', 'palm-beach' ) ) ) ),
		esc_html( $expanded ? esc_html__( 'Collapse child menu', 'palm-beach' ) : esc_html__( 'Expand child menu', 'palm-beach' ) )
	);
...
add_filter( 'walker_nav_menu_start_el', 'palm_beach_amp_menu_dropdown_toggles', 10, 4 );
